...
graficar_campo  <- function( campo)#, campo_clase, valores_clase )
{
  ...
  densidad_A  <- density( dataset[ foto_mes==202103, get(campo) ], # & get(campo_clase) %in% valores_clase
                          kernel="gaussian", na.rm=TRUE )

  densidad_B  <- density( dataset[ foto_mes==202105, get(campo) ], # & get(campo_clase) %in% valores_clase
                          kernel="gaussian", na.rm=TRUE )
...
dir.create( "./exp/DR777000b/", showWarnings = FALSE )
setwd("./exp/DR777000b/")



pdf("densidades_03_05_ajustadas.pdf")
