...
PARAM <- list()
PARAM$experimento  <- "DA777004balt"

PARAM$input$dataset       <- "./exp/FE777000b/inflation_adjusted_dataset.csv"
PARAM$input$training      <- c( 202101 )
PARAM$input$future        <- c( 202103 )

PARAM$semilla1  <- 273107 
PARAM$semilla2  <- 967693
PARAM$semillas_particion <- 3
PARAM$semillas_modelo    <- 50

PARAM$finalmodel$max_bin           <-     94
PARAM$finalmodel$max_depth         <-     -1
PARAM$finalmodel$learning_rate     <-      0.0259   #0.0142501265
PARAM$finalmodel$num_iterations    <-    359  #615
PARAM$finalmodel$num_leaves        <-   2991  #784
PARAM$finalmodel$min_data_in_leaf  <-   559  #5628
PARAM$finalmodel$feature_fraction  <-      0.438  #0.8382482539
PARAM$finalmodel$lambda_l1         <-      0.0317
PARAM$finalmodel$lambda_l2         <-      38
PARAM$finalmodel$min_gain_to_split <-      0.0
PARAM$finalmodel$semilla           <- 671017
...
for( semilla_particion in  ksemillas_particion )
{
  ...
  for( semilla_modelo  in  ksemillas_modelo )
  {
    ...
    dtrain  <- lgb.Dataset( data= data.matrix(  dataset[ train==1L, campos_buenos, with=FALSE]),
                            label= dataset[ train==1L, clase01], params = list(max_bin =  PARAM$finalmodel$max_bin ))
