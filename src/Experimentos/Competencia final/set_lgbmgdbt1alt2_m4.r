...
PARAM <- list()
PARAM$experimento <- "TE777003d_m4"
PARAM$exp_input <- "HT777003d"

# PARAM$modelos  <- 2
PARAM$modelo <- 4 # se usa el mejor de la OB, pero a futuro podria variar esto
PARAM$semilla_primos <- 273107
...
for( ksemilla in ksemillas[PARAM$indice_inicio_semilla:PARAM$indice_fin_semilla] )
{
  ...
  fwrite(tb_prediccion[, list(numero_de_cliente, foto_mes, prob, rank, clase_ternaria)],
         file = nom_resultados,
         sep = ","
  )
